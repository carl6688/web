(function ($) {
    jQuery.scrollSpeed = function (step, speed, easing) {
        var $document = $(document), $window = $(window), $body = $('html, body'), option = easing || 'default',
            root = $(window).scrollTop(), scroll = false, scrollY = $document.height() > $window.height(),
            scrollX = $document.width() > $window.width(), view;
        if (window.navigator.msPointerEnabled) return false;
        $window.on('mousewheel DOMMouseScroll', function (e) {
            var deltaY = e.originalEvent.wheelDeltaY, detail = e.originalEvent.detail;
